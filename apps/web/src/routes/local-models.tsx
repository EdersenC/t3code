...
function ModelSummary({ model }: { readonly model: LocalModelHubModel }) {
  ...
  const releaseDate = formatModelDate(model.metadata.releaseDate);
  const updatedAt = formatModelDate(model.metadata.updatedAt);
  return (
    <>
      ...
      <div className="mt-1 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
        ...
        {!releaseDate && updatedAt ? <span>Updated {updatedAt}</span> : null}
        {model.localPath ? (
          <span className="min-w-0 max-w-full truncate" title={model.localPath}>
            {model.localPath}
          </span>
        ) : null}
      </div>
