...
async function collectLocalModelPayload(inputPath: string): Promise<{
  ...
  async function visit(currentPath: string, relativePrefix: string): Promise<void> {
    const entries = await NodeFSP.readdir(currentPath, { withFileTypes: true }).catch(() => []);
    for (const entry of entries) {
      if (entry.name === ".cache" || entry.name.startsWith(".")) continue;
      const childPath = NodePath.join(currentPath, entry.name);
      ...
      if (size !== null) totalSizeBytes += size;
...
async function listHuggingFaceModels(paths: HubPaths): Promise<ReadonlyArray<LocalModelHubModel>> {
  ...
  for (const orgEntry of orgEntries) {
    ...
    for (const modelEntry of modelEntries) {
      ...
      const payload = await collectLocalModelPayload(localPath);
      if (payload.files.length === 0) continue;
      const modelId = `${orgEntry.name}/${modelEntry.name}`;
      const lowerTags: ReadonlyArray<string> = [];
      const format = inferFormatFromFilesAndTags({ files: payload.files, lowerTags });
      models.push({
        source: "huggingface",
        modelId,
        ...
        installed: true,
        format,
        ...(payload.totalSizeBytes > 0 ? { sizeBytes: payload.totalSizeBytes } : {}),
        metadata: {
          tags: [],
          recommendedDownloadPatterns: recommendedDownloadPatterns({
            files: payload.files,
            lowerTags,
          }),
          ...(payload.files.length > 0 ? { fileCount: payload.files.length } : {}),
          ...(payload.totalSizeBytes > 0 ? { totalSizeBytes: payload.totalSizeBytes } : {}),
        },
      });
    }
  }
...
function huggingFaceFileSizeBytes(files: ReadonlyArray<HuggingFaceModelFile>): number | undefined {
  let totalSizeBytes = 0;
  let hasSize = false;
  ...
  return hasSize ? totalSizeBytes : undefined;
}
...
): Promise<HuggingFaceApiModel | null> {
  const response = await fetch(huggingFaceModelApiUrl(modelId), {
    headers: huggingFaceHeaders(token),
  });
...
function mapHuggingFaceApiModel(
  ...
  const files = Array.isArray(raw.siblings)
    ...
    : [];
  const fileSizeBytes = Array.isArray(raw.siblings)
    ? huggingFaceFileSizeBytes(raw.siblings.filter(isHuggingFaceModelFile))
    : undefined;
  const totalSizeBytes = typeof raw.usedStorage === "number" ? raw.usedStorage : fileSizeBytes;
  ...
  return {
    ...
    format,
    ...(totalSizeBytes !== undefined ? { sizeBytes: totalSizeBytes } : {}),
    metadata: {
      tags,
      ...
      ...(typeof raw.createdAt === "string" ? { releaseDate: raw.createdAt } : {}),
...
export const localModelHubTestExports = {
  listHuggingFaceModels,
  mapHuggingFaceApiModel,
};
  ...
  const sourceAdapters: Record<LocalModelHubSource, LocalModelSourceAdapter> = {
    huggingface: {
      ...
      searchModels: async (searchInput, paths) => {
        ...
        const rawModels = Array.isArray(payload) ? (payload as ReadonlyArray<unknown>) : [];
        const detailToken = process.env.HF_TOKEN;
        const hydratedModels = await Promise.all(
          rawModels.map(async (raw) => {
            ...
            const modelId =
              typeof base.id === "string"
                ...
                : typeof base.modelId === "string"
                  ? base.modelId
                  : "";
            if (modelId.length === 0) return base;
            const detail = await fetchHuggingFaceModelDetail(modelId, detailToken).catch(
              () => null,
            );
            return detail ?? base;
          }),
        );
        const remoteModels = hydratedModels
          .map((raw) => mapHuggingFaceApiModel(raw, paths, installedIds))
          .filter((model): model is LocalModelHubModel => model !== null);
        return {
